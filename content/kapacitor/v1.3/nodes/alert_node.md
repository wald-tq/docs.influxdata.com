...
an alert is specified via a [lambda expression](/kapacitor/v1.3/tick/expr/).
...
|:----------|:--|
| **[alert](#example)()** | Chaining method takes no arguments. |

**Alias Constructor**

| Signature | Description |
|:----------|:------------|
|  **[deadman](#deadman)( threshold `float64`, interval `duration`, exprs `...lambda`)** |_threshold_ is the threshold at which an alert will be triggered.<br/>_interval_ indicates how often to check the throughput.<br/>_exprs_ is an optional list of expressions to also be evaluated. |

**Property Methods**

| Setters | Description |
|:-----------|:---|
| <br/>**Data Properties**<br/><br/> |
...
| **[stateChangesOnly](#statechangesonly)()** | Has no arguments. Sets a flag indicating that events will only be sent when the state has changed. |
| <br/>**Thresholds**<br/><br/> |
| **[info](#info)( expr `lambda`)** | _expr_ is the filter expression for the "Info" alert level. |
| **[warn](#warn)( expr `lambda`)** | _expr_ is the filter expression for the "Warning" alert level. |
| **[crit](#crit)( expr `lambda`)** | _expr_ is the filter expression for the "Critical" alert level. |
| **[infoReset](#inforeset)( expr `lambda`)** | _expr_ is the filter expression used to reset the "Info" alert level to a lower level. |
| **[warnReset](#warnreset)( expr `lambda`)** | _expr_ is the filter expression used to reset the "Warning" alert level to a lower level. |
| **[critReset](#critreset)( expr `lambda`)** | _expr_ is the filter expression used to reset the "Critical" alert level to a lower level. |
| **[flapping](#flapping)( low `float64`, high `float64`)** | Performs flap detection on alerts.<br/>_high_ is the percentage change threshold that indicates a "flapping" state.<br/>_low_ is the percentage change threshold needed to leave the "flapping" state. |
| **[history](#history)( count `int64`)** | _count_ is the number of previous states to retain when computing flapping levels. |
| <br/>**Event Handlers**<br/><br/> | <br/>_Note that many event handlers contain internal properties that can also be set._ |
| **[log](#log)( filepath `string`)** | _filepath_ is the path to the file, where the alert should be logged. |
...
| **[opsGenie](#opsgenie)()** | Has no arguments. Returned handler is configured using internal properties and through the Kapacitor configuration file. |
| **[pagerDuty](#pagerduty)()** | Has no arguments. Returned handler is configured using internal properties and through the Kapacitor configuration file. |
| **[pushover](#pushover)()** | Has no arguments. Returned handler is configured using internal properties and through the Kapacitor configuration file. |
| **[sensu](#sensu)()** | Has no arguments. Returned handler is configured using internal properties and through the Kapacitor configuration file. |
| **[slack](#slack)()** | Has no arguments. Returned handler is configured using internal properties and through the Kapacitor configuration file. |
| **[talk](#talk)()** | Has no arguments. Configured in Kapacitor configuration file. |
| **[telegram](#telegram)()** | Has no arguments. Returned handler is configured using internal properties and through the Kapacitor configuration file. |
| **[victorOps](#victorops)()** | Has no arguments. Returned handler is configured using internal properties and through the Kapacitor configuration file. |
...
In the Kapacitor configuration file some handlers like Email, HipChat, Sensu, Slack, OpsGenie, VictorOps, PagerDuty, Telegram and Talk have a configuration
