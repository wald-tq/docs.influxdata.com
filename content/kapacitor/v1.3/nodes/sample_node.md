...
|:----------|:--|
| **[sample](#example)( rate `interface{}`)** | _rate_ is the rate at which to sample points from the data.  This can be an `int64` to keep every Nth point or batch.  This can also be a `duration` to keep one point or batch for every interval. |
...
One point or batch will be provided every count or duration specified.
...
-	[InfluxDBOut](/kapacitor/v1.3/nodes/sample_node/#influxdbout)
