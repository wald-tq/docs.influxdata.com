...
[Queries](../glossary/#query) that use field values as filters must scan all values that match the other conditions in the query.
...
To optimize your queries, it may be beneficial to rearrange your [schema](../glossary/#schema) such that the fields (`butterflies` and `honeybees`) become the tags and the tags (`location` and `scientist`) become the fields:
...
If you're interested in reading more about retention policies, check out [Database Management](../../query_language/database_management/#retention-policy-management).
...
Understanding the concept of a series is essential when designing your [schema](../glossary/#schema) and when working with your data in InfluxDB.
...
See [Authentication and Authorization](../../query_language/authentication_and_authorization/) and [Continuous Queries](../../query_language/continuous_queries/) for more on those topics.
...
If you're just starting out, we recommend taking a look at [Getting Started](../../introduction/getting_started/) and the [Writing Data](../../guides/writing_data/) and [Querying Data](../../guides/querying_data/) guides.
